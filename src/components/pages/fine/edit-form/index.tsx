...
function EditFineForm({ fine }: { fine: Fine }) {
  const [loading, setLoading] = useState(false);
  const [done, setDone] = useState(false);
  const [oriVehicleRes, setOriVehicleRes] = useState<ResVehicle | null>(null);

  useEffect(() => {
    const run = async () => {
      const resVehi = await getVehicle(fine.finedVehicle);
      if (resVehi.error) return window.alert('Ha ocurrido un error');
      setOriVehicleRes(resVehi.result);
    };
    run();
  }, []);
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = async e => {
    ...
    const newFine: PostFine = {
      reason: reason.toString(),
      fined_cit: citizenRes.result.id,
      fined_vehicle: vehicleRes.result.id,
    };

    const newFineInfo = getObjectDifferences(fine, newFine);

    const res = await editFine(fine.id, newFineInfo);
              ...
              <Form.Field name='plate'>
                <div className='flex items-baseline justify-between'>
                  ...
                  <Form.Message
                    className='text-sm text-red-400 opacity-[0.8]'
                    match={async value => {
                      const res = await getVehicleByPlate(value);
                      return res.result === null;
                    }}
                  >
                    Por favor introduce una placa de un vehiculo registrado
                  </Form.Message>
                </div>
                <Form.Control asChild>
                  <input
                    ...
                    defaultValue={oriVehicleRes?.plate}
                    required
                  />
                </Form.Control>
              </Form.Field>

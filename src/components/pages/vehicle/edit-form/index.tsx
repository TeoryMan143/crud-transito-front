...
import type { PostVehicle, ResCitizen, Vehicle } from '../../../../lib/types';
import { CarIcon } from '../../../icons/car';
import { MotorcycleIcon } from '../../../icons/motor-cycle';
import { checkImageUrl, getObjectDifferences } from '../../../../lib/utils';

function EditVehicleForm({ vehicle }: { vehicle: Vehicle }) {
  const [loading, setLoading] = useState(false);
  const [vType, setVType] = useState<string>(vehicle.vtype);
  const [done, setDone] = useState(false);
  const classItems =
    'flex flex-col items-center data-[state=on]:text-red-500 data-[state=on]:font-bold';
  const [citizenRes, setCitizenRes] = useState<ResCitizen | null>(null);

  useEffect(() => {
    const run = async () => {
      const resCit = await getCitizen(vehicle.owner);
      if (resCit.error) return window.alert('Ha ocurrido un error');
      setCitizenRes(resCit.result);
    };
    run();
  }, []);
  ...
  const handleSubmit: FormEventHandler<HTMLFormElement> = async e => {
    ...
    const newVehiInfo = getObjectDifferences(vehicle, vehi);

    const res = await editVehicle(vehicle.id, newVehiInfo);
                ...
                <Form.Field name='owner' className='flex-1'>
                  ...
                  <Form.Control asChild>
                    <input
                      className='box-border w-full bg-red-500 shadow-white inline-flex h-[35px] appearance-none items-center justify-center rounded-[4px] px-[10px] text-[15px] leading-none text-white shadow-[0_0_0_1px] outline-none hover:shadow-[0_0_0_1px_white] focus:shadow-[0_0_0_2px_white] selection:color-white selection:bg-white'
                      type='number'
                      defaultValue={citizenRes?.cedula}
                      required
                    />
                  </Form.Control>
                </Form.Field>
                ...
                <Form.Field name='soat_end' className='flex-1'>
                  ...
                  <Form.Control asChild>
                    <input
                      className='box-border w-full bg-red-500 shadow-white inline-flex h-[35px] appearance-none items-center justify-center rounded-[4px] px-[10px] text-[15px] leading-none text-white shadow-[0_0_0_1px] outline-none hover:shadow-[0_0_0_1px_white] focus:shadow-[0_0_0_2px_white] selection:color-white selection:bg-white'
                      type='date'
                      defaultValue={vehicle.soatEnd.toISOString().split('T')[0]}
                      required
                    />
                  </Form.Control>
                </Form.Field>
                <Form.Field name='tech_end' className='flex-1'>
                  ...
                  <Form.Control asChild>
                    <input
                      className='box-border w-full bg-red-500 shadow-white inline-flex h-[35px] appearance-none items-center justify-center rounded-[4px] px-[10px] text-[15px] leading-none text-white shadow-[0_0_0_1px] outline-none hover:shadow-[0_0_0_1px_white] focus:shadow-[0_0_0_2px_white] selection:color-white selection:bg-white'
                      type='date'
                      defaultValue={vehicle.techEnd.toISOString().split('T')[0]}
                      required
                    />
                  </Form.Control>
                </Form.Field>
